...
* **Design gate:** Resolved by DEC-014 — EasyMail Design System v1 adopted for landing/marketing surfaces (see progress note below). DEC-013 still governs the authenticated product visual direction; Geist Sans and `#1F5FA9` remain explicitly provisional there pending a dedicated authenticated-app design pass.
* **Last updated:** 2026-09-13 (Phase 12 approved by the product owner and merged to `main`; `ui/design-system-v1` — Design System v1 landing-page redesign and motion system — merged into `main` in the same integration)

### Design System v1 progress (originally `ui/design-system-v1`, now merged into `main`)

> The entries below are carried over unedited from the `ui/design-system-v1` branch's own progress log as of its last update (2026-09-12), preserved as a historical record of that branch's work. They have not been independently re-verified as part of this merge.
...
* Passes B–G (shell, Recap/Triage/detail, Settings/states, auth/onboarding, landing rebuild, responsive/accessibility) not yet started as of that branch update — note that this merge's own diff shows some authenticated-app component files (`account-row.tsx`, `filter-nav.tsx`, `provider-mark.tsx`, `correction-form.tsx`, triage/settings pages) were touched later in the branch's history; that work has not been reviewed or verified as part of this integration and should be audited in a follow-up pass rather than assumed complete.
* The landing-page/marketing rebuild (hero, motion system, product preview, feature showcase, integrations, trust, security, FAQ, final CTA, navbar, and a new trusted-by strip) was completed after this log entry and is merged into `main` as of this integration — see the checkpoint commits on the (now-merged) `ui/design-system-v1` branch for full detail.

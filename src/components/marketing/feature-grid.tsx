...
function RecapSkeleton() {
  return (
    <div className="flex h-full flex-col justify-center gap-3 px-5 py-5 sm:px-7">
      <div className="flex items-center justify-center gap-3">
        <span className="inline-flex items-center gap-1.5 rounded-full border border-border bg-surface px-2.5 py-1 text-[11px] font-medium text-text-muted">
          <Image alt="" aria-hidden="true" height={13} src="/hero/gmail.svg" width={16} />
          Gmail
        </span>
        <span className="inline-flex items-center gap-1.5 rounded-full border border-border bg-surface px-2.5 py-1 text-[11px] font-medium text-text-muted">
          <Image
      ...
      <ul className="overflow-hidden rounded-[10px] border border-border-strong bg-surface">
        {recapRows.map((row) => (
          <li
            ...
            key={row.subject}
          >
            <p className="truncate text-[12px] font-medium text-text">{row.subject}</p>
...
function TriageSkeleton() {
  return (
    <div className="flex h-full flex-col justify-center gap-2 px-5 py-6 sm:px-7">
      <div className="rounded-[10px] border border-primary/25 bg-primary-subtle px-3.5 py-3">
        <div className="flex items-center justify-between gap-3">
          <p className="text-[12px] font-medium text-text">Can you review the brief?</p>
          <IntentLabel intent="needs_reply" />
        </div>
        <p className="mt-1 font-mono text-[9px] uppercase tracking-[0.06em] text-primary">
          In focus
        </p>
      </div>
...
function JudgmentSkeleton() {
  return (
    <div className="flex h-full flex-col items-center justify-center gap-3 px-5 py-6 sm:px-7">
      <div className="w-full max-w-[220px] rounded-[10px] border border-border bg-surface px-3.5 py-3 opacity-60">
      ...
      <div className="w-full max-w-[220px] rounded-[10px] border border-primary/25 bg-primary-subtle px-3.5 py-3">
        <span className="font-mono text-[9px] uppercase tracking-[0.06em] text-primary">
          You corrected to
        </span>
        <div className="mt-2">
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between gap-3 px-5 py-4 sm:px-6">
        <span className="inline-flex items-center gap-2 text-[15px] font-semibold tracking-[-0.01em] text-text">
          <Icon aria-hidden="true" className="size-4 text-primary" strokeWidth={1.8} />
          {title}
        </span>
        <span
          aria-hidden="true"
          className="flex size-6 shrink-0 items-center justify-center rounded-full border border-border text-text-subtle transition-colors group-hover:border-primary/40 group-hover:text-primary"
        >
  ...
  return (
    <section className="border-b border-border bg-background">
      <div className="mx-auto max-w-6xl px-6 py-24 lg:px-8 lg:py-28">
        ...
        <RevealGroup
          as="div"
          className="mt-14 grid gap-6 lg:mt-16 lg:grid-cols-3"
          staggerChildren={0.1}
        >
